...
struct Sum
{
    ...
    inline T operator()(thread const T &a, thread const U &b) const { return a + b; }
    template <typename T, typename U>
    inline T operator()(threadgroup const T &a, threadgroup const U &b) const { return a + b; }
                                      ...
                                      const U substitution_value)
{
    ...
    for (ushort i = 1; i < GRAIN_SIZE; i++)
    {
        value = Op(value, (lid * GRAIN_SIZE + i < n) ? input_data[lid * GRAIN_SIZE + i] : substitution_value);
    }
}
...
template <ushort BLOCK_SIZE, typename OPERATION, typename T, bool BROADCAST>
static T
...
template <ushort BLOCK_SIZE, typename OPERATION, typename T, bool BROADCAST = false>
static T
...
template <ushort BLOCK_SIZE, ushort GRAIN_SIZE, typename OPERATION = Sum, typename T, typename U, typename V>
kernel void
                   ...
                   ushort local_id [[thread_index_in_threadgroup]])
{
    ...
    // write result for this threadgroup to global memory
    if (local_id == 0)
        atomic_fetch_add_explicit(output_data, value, memory_order_relaxed);
}

// Explicit specializations of `ReduceKernel<T,U,V>` with [T=input, U=intermediary, V=output]
